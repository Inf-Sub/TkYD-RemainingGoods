...
class DatabaseManager:
    def __init__(self, host, port, database, user, password, schema_dir):
        self.host = host
        self.port = port
        self.user = user
    ...
    async def create_tables(self) -> None:
        async with self.conn.cursor() as cur:
            schema_json_path = join(self.schema_dir, 'schema_order.json')

            if Path(schema_json_path).exists():
                # Загрузка порядка файлов 'schema' из JSON
                async with async_open(schema_json_path, 'r', encoding='utf-8') as f:
                    content = await f.read()
                    schema_files_list = json_loads(content)['order']
            else:
                # Загрузка файлов 'schema' из директории (алфавитный порядок)
                schema_files_list = [
                    filename for filename in listdir(self.schema_dir)
                    if filename.startswith('schema') and filename.endswith('.sql')
                ]

            tables = await self._load_schemas(schema_files_list)
    ...
    async def _load_schemas(self, order: list) -> dict:
        ...
        for filename in order:
            # Удаляем расширение файла и префикс 'schema_'
    ...
    async def _create_table_if_not_exists(cur, table, schema) -> None:
        # Сначала нужно проверить существование таблицы
        ...
        if result:
            ...
            # Можно здесь же инициализировать данные, если это необходимо
            # if table == 'stores':
            #     await self.initialize_data_stores(DB_INIT_DATA_STORES_PATH)


async def main():
    # conn = await aiomysql.connect(user='user', password='pass',
    #                               db='dbname', host='127.0.0.1', port=3306)

    # schema_dir = DATA_DIR
    loop = asyncio.get_event_loop()
    db_manager = DatabaseManager(
        host=DB_HOST, port=DB_PORT, database=DB_NAME, user=DB_USERNAME, password=DB_PASSWORD, schema_dir=DATA_DIR)
